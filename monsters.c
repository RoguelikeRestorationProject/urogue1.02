...
new_monster(struct linked_list *item, int type, coord *cp, int max_monster)
{
    ...
    register int i;
	register int min_intel, max_intel;
    ...
    tp->t_no_move = 0;
    tp->t_doorgoal = -1;
    memmove(&tp->t_oldpos, cp, sizeof (coord));
    ...
    turn_on(*tp, ISREADY); /* ready for action this turn! */
    for (i=0; i<10; i++)
...
{
    register struct linked_list *item;
    register int i, j, min_worth, nitems, goods = 0, chance, which_item, w;
